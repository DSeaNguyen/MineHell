...
public class Controller : MonoBehaviour
{
    ...
    /// </summary>
    public enum AimModes { AimTowardsMouse, AimFlipWithMouse, AimForwards };
    ...
    private bool canAimWithMouse
    {
        get
        {
            return aimMode == AimModes.AimTowardsMouse || aimMode == AimModes.AimFlipWithMouse;
        }
    }
    ...
    {
        if (Time.timeScale <= 0) return; // game đang pause thì bỏ qua

        if (Camera.main == null)
        {
            Debug.LogWarning("Main Camera is missing! Cannot calculate look direction.");
            return;
        }

        Vector3 mouseWorldPos = Camera.main.ScreenToWorldPoint(point);
        mouseWorldPos.z = 0; // tránh lỗi lệch trục z

        if (aimMode == AimModes.AimFlipWithMouse)
        {
        ...
        else if (aimMode == AimModes.AimForwards)
        {
            if (myRigidbody != null)
            {
